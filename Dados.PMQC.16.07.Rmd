...
library(data.table)
mypath.1 <- "/Volumes/NO NAME/ExplAnalysis_PMQC/Dados.PMQC.16.07.pmqc.dt.csv"
mypath.2 <- "/Volumes/NO NAME/ExplAnalysis_PMQC/Dados.PMQC.16.07.pmqc.un.dt.csv"
...
for(i in prods) {
        hist(t50rec.dt[Produto==i,V1],main=i,
             ...
             breaks=seq(floor(min(t50rec.dt[,V1])),
                        ceiling(max(t50rec.dt[,V1])),length.out=60))
}
par(op)
```
...
for(i in prods) {
        hist(t85rec.dt[Produto==i,V1],main=i,
             ...
             breaks=seq(floor(min(t85rec.dt[,V1])),
                        ceiling(max(t85rec.dt[,V1])),length.out=2000))
}
par(op)
```
...
for(i in prods) {
        hist(t90rec.dt[Produto==i,V1],main=i,
             ...
             breaks=seq(floor(min(t90rec.dt[,V1])),
                        ceiling(max(t90rec.dt[,V1])),length.out=60))
}
par(op)
```
...
for(i in prods) {
        hist(t95rec.dt[Produto==i,V1],main=i,
             ...
             breaks=seq(floor(min(t95rec.dt[,V1])),
                        ceiling(max(t95rec.dt[,V1])),length.out=400))
}
par(op)
```
...
for(i in prods) {
        hist(ce.dt[Grupo.produto==i,V1],main=i,
             ...
             breaks=seq(floor(min(ce.dt[,V1])),
                        ceiling(max(ce.dt[,V1])),length.out=200))
}
par(op)
```
...
**Cinco maiores valores de teor de hidrocarbonetos:**

```{r thc.2,echo=FALSE,cache=TRUE,warning=FALSE}
thc.dt[order(-as.numeric(Resultado)),.(Contratada,Id.text,Resultado)][1:5,]
```
...
**Cinco maiores valores de teor de metanol:**

```{r TMet.2,echo=FALSE,cache=TRUE,warning=FALSE}
tmet.dt[order(-as.numeric(Resultado)),.(Contratada,Id.text,Resultado,Grupo.produto)
        ][1:5,]
```
...
hist(s.dt[Produto=="DIESEL B S10",V1],main="DIESEL B S10",
     ...
     breaks=seq(floor(min(s.dt[Produto=="DIESEL B S10",V1])),
                ceiling(max(s.dt[Produto=="DIESEL B S10",V1])),length.out=30)
     )
...
pmqc.un.dt[Produto %in% c("OLEO DIESEL B S10 COMUM",
                          "OLEO DIESEL B S10 ADITIVADO") & Ensaio.=="Enxofre",
           .(Id.text,Resultado)][order(-as.numeric(Resultado))][1:5]
...
pmqc.un.dt[Produto %in% c("OLEO DIESEL B S500 COMUM",
                          "OLEO DIESEL B S500 ADITIVADO") & Ensaio.=="Enxofre",
           .(Id.text,Resultado)][order(-as.numeric(Resultado))][1:5]
...
**Cinco maiores valores para teor de marcador:**

```{r marcador.2,echo=FALSE,cache=TRUE,warning=FALSE}
marcador.dt[order(-Resultado)][1:5]
```
...
# agualivre.dt <- pmqc.un.dt[Ensaio.=="AguaLivre",as.numeric(Resultado),
#                       keyby=Produto]
# 
# # sum(is.na(agualivre.dt))
# # agualivre.dt[is.na(V1),V1:=0]
# aggregate(agualivre.dt$V1,by=list(agualivre.dt$Produto),FUN=summary)
# 
# prods <- agualivre.dt[,unique(Produto)]
# for(i in prods) {
#         hist(agualivre.dt[Produto==i,V1],main=paste("Agua Livre:",i),
#              xlab="Agua Livre, % volume")
# }
